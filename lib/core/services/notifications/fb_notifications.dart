...
import 'package:parent/features/shared/dialogs/child_checkin.dart';
...
class FBNotifications implements Disposable{
  ...
  _listenToForegroundNotification() {
    FirebaseMessaging.onMessage.listen((RemoteMessage message) {
      ...
      if (message.notification != null) {
        if (_isChildLocationUpdate(message)) {
          final BuildContext? context =
              Routes.navigatorKey.currentState?.context;
          final Student student = Student.fromJson(message.data['student']);
          if (context != null) {
            /// show notification dialog if notification is child check in/out
            /// if context is available via [Routes.navigatorKey]. else show
            /// the default notification.
            showDialog(
                context: context,
                builder: (_) {
                  return ChildChekInDialog(
  ...
  bool _isChildLocationUpdate(RemoteMessage message) {
    return message.data.containsKey('type') &&
        (message.data['type'] == AppStrings.childCheckInNotification ||
            message.data['type'] == AppStrings.childCheckOutNotification);
  }
